...
export default function BlogPostShareImage({ data, pageContext }) {
	...
	const [background] = useQueryParam("background", StringParam);
	...
	const radioButton = value => {
		return (
			<label className="mx-1" htmlFor={`radio-${value}`}>
				<input
					type="radio"
					id={`radio-${value}`}
					value={value}
					checked={socialMedia === value}
					onChange={() => setSocialMedia(value)}
				/>
	...
	const setColorAndBackground = () => {
		const _colors = getColor(color);
		const _bgImage = getBackgroundImage(
			_colors.backgroundImage,
			0.4,
			background
		);

		setColors(_colors);
		setBackgroundImage(_bgImage);
	};

	useEffect(() => {
		if (!socialMedia) setSocialMedia("twitter");
		setSizes(getSizes(socialMedia));
		if (!sizes) setColorAndBackground();
	}, [socialMedia, setSizes]);

	if (!colors || !sizes || !backgroundImage) return <div />;
	...
	return (
		<div>
			...
			>
				<div
					className="p-4 m-4 flex flex-col justify-between rounded-lg shadow-lg opacity-75 w-full"
					style={{
